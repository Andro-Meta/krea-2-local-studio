...
async def import_records(urls: list[str], *, max_pages: int, browser_discovery: bool) -> int:
    ...
    records = crawler.crawl(seeds, max_pages=max_pages)
...
def main() -> None:
    ...
    imported = asyncio.run(import_records(args.urls, max_pages=args.max_pages, browser_discovery=args.browser))
