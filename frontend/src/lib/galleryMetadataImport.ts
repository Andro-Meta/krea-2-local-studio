...
export function metadataToGenerateParams<TMode extends ImportTargetMode>(
  ...
  const patch: Partial<GenerateParams> & { mode: TMode } = {
    ...
    sampler: oneOf(metadata.sampler, ['euler', 'euler_flow', 'euler_ancestral', 'euler_ancestral_cfg_pp', 'euler_cfg_pp', 'exp_heun_2_x0_sde', 'lcm', 'dpmpp_2m', 'ddim', 'uni_pc'] as const),
    scheduler: oneOf(metadata.scheduler, ['simple', 'normal', 'beta', 'sgm_uniform', 'karras', 'exponential'] as const),
