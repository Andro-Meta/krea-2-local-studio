...
export function metadataToGenerateParams<TMode extends ImportTargetMode>(
  ...
  const patch: Partial<GenerateParams> & { mode: TMode } = {
    ...
    inpaint_method: oneOf(metadata.inpaint?.method, ['native', 'lanpaint_experimental'] as const),
    ...
    edit_provider: oneOf(metadata.edit_provider || metadata.resolved_provider, ['auto', 'krea_native'] as const),
    ...
    seed_variance_schedule: oneOf(metadata.seed_variance?.schedule, ['constant', 'decreasing', 'step_cutoff', 'hard_lock', 'tiered_release'] as const),
    ...
    vae_degrid: booleanValue(metadata.vae_degrid),
